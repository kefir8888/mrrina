...
class Skeleton (Modality):
    ...
    def _process_data (self, frame = None):
        ...
        for kp in necessary_keypoints_names:
            ...
            print(self.depth_img[ind])
            kps.update ({kp : np.asarray([self.read_data [ind * 2], self.read_data [ind * 2 + 1], self.depth_img[ind]])})

        #    kps.update ({kp : np.asarray([self.read_data [ind * 2], self.read_data [ind * 2 + 1], self.depth_img[self.read_data [ind * 2+1], self.read_data [ind * 2]]])})

        def get_vec(dot1, dot2):
            vec = np.asarray([dot2[0]-dot1[0],dot2[1]-dot1[1],dot2[2]-dot1[2]])
            den = math.sqrt(vec[0]**2 + vec[1]**2 + vec[2]**2)
            return vec/den, math.sqrt(sum(np.power(vec,2)))


        def get_mod_vec(vec):
            if sum(np.power(vec,2)) < 0:
                print("AAAAAAAAAA")
                print(sum(np.power(vec,2)))
                print(np.power(vec,2))
                print(int(vec[0]), int(vec[1]), int(vec[2]))
            return math.sqrt(sum(np.power(vec,2)))

        mid_hip = [int((kps ["r_hip"][0] + kps ["l_hip"][0])/2), int((kps ["r_hip"][1] + kps["l_hip"][1])/2), int((kps ["r_hip"][2] + kps["l_hip"][2])/2)]
        hip_neck, mod_hip_neck = get_vec(mid_hip,kps["neck"])
        neck_l_sho, mod_neck_l_sho = get_vec(kps["neck"], kps["l_sho"])
        l_sho_elb, mod_l_sho_elb = get_vec(kps["l_sho"], kps["l_elb"])
        #
        N_256 = np.cross(neck_l_sho, l_sho_elb)
        N_125 = np.cross(neck_l_sho, hip_neck)
        R_lut = np.cross(hip_neck, N_125)

        mod_R_lut = get_mod_vec(R_lut)

        # print(get_mod_vec(R_lut))

        theth_lsp = math.acos(np.dot(mid_hip,np.cross(R_lut,l_sho_elb))/get_mod_vec(mid_hip)/get_mod_vec(np.cross(R_lut, l_sho_elb)))
        theth_lsr = 1.57 - math.acos(np.dot(l_sho_elb, R_lut)/(mod_R_lut * mod_l_sho_elb))
        self.logger.update("PITCH ", theth_lsp)
        self.logger.update("ROLL ", theth_lsr)
        # mult_vec_lut_sh = np.cross(R_lut,l_sho_elb)
        # mod = get_mod_vec(mult_vec_lut_sh)
        # mod_1 = get_mod_vec(hip_neck)
        # mod_2 = get_mod_vec(l_sho_elb)
        # if mod != 0 and mod_1 != 0 and mod_2 != 0:
        #     self.logger.update("PITCH ", np.dot(mid_hip,mult_vec_lut_sh)/(mod*mod_1))
        #     self.logger.update("ROLL ", (1.57 - np.dot(l_sho_elb,mult_vec_lut_sh)/(mod*mod_2)))
        ...
        # kps["neck"], kps["l_sho"], kps["l_wri"]




        # self.logger.update("WRIST x", kps["r_wri"][0])
        # self.logger.update("WRIST y", kps["r_wri"][1])
        # self.logger.update("WRIST z", self.depth_img[kps["r_wri"][1], kps["r_wri"][0]])
        #
        # self.logger.update("WRIST_ x", kps["r_elb"][0])
        # self.logger.update("WRIST_ y", kps["r_elb"][1])
        # self.logger.update("WRIST_ z", self.depth_img[kps["r_elb"][1], kps["r_elb"][0]])
        ##########################################3d block#########################################################################
        ...
        #
        # r_sh_elb_xy = (self.poses_3d[0][10][0] - self.poses_3d[0][9][0], self.poses_3d[0][10][2] - self.poses_3d[0][9][2])
        # r_sh_elb_yz = (self.poses_3d[0][10][2] - self.poses_3d[0][9][2], self.poses_3d[0][10][1] - self.poses_3d[0][9][1])
        #
        # r_elb_wrist_xy = (self.poses_3d[0][11][0] - self.poses_3d[0][10][0], self.poses_3d[0][11][2] - self.poses_3d[0][10][2])
        # r_elb_wrist_yz = (self.poses_3d[0][11][2] - self.poses_3d[0][10][2], self.poses_3d[0][11][1] - self.poses_3d[0][10][1])
        #
        # self.logger.update("SHOULDER xy", common.angle_2_vec (neck_hip_xy, r_sh_elb_xy))
        # self.logger.update("SHOULDER yz", common.angle_2_vec (neck_hip_yz, r_sh_elb_yz))
        # self.logger.update("WRIST xy", common.angle_2_vec (r_sh_elb_xy, r_elb_wrist_xy))
        # self.logger.update("WRIST yz", common.angle_2_vec (r_sh_elb_yz, r_elb_wrist_yz))
        # self.logger.update("SHOULDER WRIST", common.angle_2_vec (r_sh_elb_xy, r_elb_wrist_xy))



        ##########################################################################################################################3
        # hips_mid_xy  = (int((kps ["r_hip"] [0] + kps ["l_hip"] [0]) / 2), int((kps ["r_hip"] [1] + kps ["l_hip"] [1]) / 2))
        # hips_mid_z = self.depth_img[hips_mid_xy[1],hips_mid_xy[0]]
        #
        # neck_hip_xy  = (hips_mid_xy[0] - kps ["neck"][0],  hips_mid_xy[1] - kps ["neck"][1])
        # neck_hip_z = int(self.depth_img[hips_mid_xy[1], hips_mid_xy[0]]) - int(self.depth_img[kps ["neck"][1],kps ["neck"][0]])
        #
        # r_sh_elb_xy  = (kps ["r_elb"][0] - kps ["r_sho"][0], kps ["r_elb"][1] - kps ["r_sho"][1])
        # r_sh_elb_z = int(self.depth_img[kps ["r_elb"][1],kps ["r_elb"][0]]) - int(self.depth_img[kps ["r_sho"][1],kps ["r_sho"][0]])
        #
        #
        # r_elb_wri_xy  = (kps ["r_wri"][0] - kps ["r_elb"][0], kps ["r_wri"][1] - kps ["r_elb"][1])
        # r_elb_wri_z = int(self.depth_img[kps ["r_wri"][1],kps ["r_wri"][0]]) - int(self.depth_img[kps ["r_elb"][1],kps ["r_elb"][0]])
        #
        # self.logger.update("x_wrist", kps ["r_elb"][0] - kps ["r_sho"][0])
        # self.logger.update("y_wrist", kps ["r_elb"][1] - kps ["r_sho"][1])
        # self.logger.update("z_wrist", int(self.depth_img[kps ["r_elb"][1],kps ["r_elb"][0]]) - int(self.depth_img[kps ["r_sho"][1],kps ["r_sho"][0]]))
        # self.logger.update("angle", common.angle_2_vec(r_sh_elb_xy, r_elb_wri_xy))
        # self.logger.update("angle_z", common.angle_2_vec((r_sh_elb_xy[1],r_sh_elb_z), (r_elb_wri_xy[1],r_elb_wri_z)))
        # self.logger.update("Z_5", common.angle_2_vec(neck_hip_xy, r_sh_elb_xy))

        # self.logger.update("Z_0", round(common.angle_2_vec ((neck_hip_xy[1],neck_hip_z), (r_sh_elb_xy[1], r_sh_elb_z)),2))
        ...
        #
        # self.logger.update("R_shoulder", common.angle_2_vec (neck_hip, r_sh_elb_xy))
        ...
        if (frame is not None):
            cv2.arrowedLine (frame, (int (mid_hip[0]), int (mid_hip[1])),
...
class Video (Modality):
    def __init__ (self, video_path_ = "", model_path_ = "", mode_ = "GPU", base_height_ = 512, logger_ = 0):
        ...
        if (video_path_ == ""):
            ...
            # self.all_data = cv2.VideoCapture("/home/kompaso/Desktop/r_hand.mp4")
            self.all_data = cv2.VideoCapture(0)
            # self.all_data = self.all_data .wait_for_frames()
    ...
    def _infer_net (self, frame):
        ...
        if len(poses_3d):
            ...
            x = poses_3d_copy[:, 0::4]
            y = poses_3d_copy[:, 1::4]
            self.depth_img = poses_3d_copy[:, 2::4]
            z = self.depth_img
    ...
    def _process_data(self):
        if sum (self.read_data) != -36 and self.read_data != []:
            ...
            self.skel.depth_img = self.depth_img[0]
...
class RealSense (Modality):
    def __init__ (self, video_path_ = "", model_path_ = "", mode_ = "GPU", base_height_ = 512, logger_ = 0):
        ...
        self.net = PoseEstimationWithMobileNet_()
        checkpoint = torch.load("models/checkpoint_iter_370000.pth", map_location=torch.device('cuda'))
        load_state(self.net, checkpoint)
    ...
    def _read_data (self):
        if (self.read == False):
            ...
            self.skel.depth_img = np.asanyarray(aligned_frames.get_depth_frame().get_data())
        ...
        # self.read_data, _ = self.video_skel._infer_net (self.img) #draww(self.img)
        self.read_data, self.frame =  get_skel_coords(self.net, self.img, 512, False, 1, 1)


    def _process_data(self):
        if sum (self.read_data) != -36 and self.read_data != []:
            self.skel.read_data = self.read_data
            # self.skel.poses_3d = self.video_skel.poses_3d
            self.skel._process_data(self.frame)
            self.processed_data = self.skel.processed_data
